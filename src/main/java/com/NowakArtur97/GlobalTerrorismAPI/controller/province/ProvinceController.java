...
public class ProvinceController extends GenericRestControllerImpl<ProvinceModel, ProvinceDTO, ProvinceNode> {
        ...
        public ResponseEntity<ProvinceModel> findById(
                @ApiParam(value = "Province's id value needed to retrieve details", name = "id", type = "integer", required = true, example = "1") @PathVariable("id") Long id) {
        ...
        public ResponseEntity<ProvinceModel> update(
                @ApiParam(value = "Id of the Province being updated", name = "id", type = "integer", required = true, example = "1") @PathVariable("id") Long id,
        ...
        @Override
        @ApiOperation(value = "Update Province' fields using Json Patch", notes = "Update Province's fields using Json Patch", consumes = PatchMediaType.APPLICATION_JSON_PATCH_VALUE)
        @ApiResponses({
        ...
        public ResponseEntity<ProvinceModel> updateFields(
                @ApiParam(value = "Id of the Province being updated", name = "id", type = "integer", required = true, example = "1") @PathVariable("id") Long id,
                @ApiParam(value = "Province's fields to update", name = "province", required = true) @RequestBody JsonPatch objectAsJsonPatch) {
...
//     uniqueness)
        ...
        @ApiResponses({
                ...
                @ApiResponse(code = 400, message = "Incorrectly entered data", response = ErrorResponse.class)})
        public ResponseEntity<ProvinceModel> updateFields(
                @ApiParam(value = "Id of the Province being updated", name = "id2", type = "integer", required = true, example = "1") @PathVariable("id2") Long id,
                @ApiParam(value = "Province's fields to update", name = "province", required = true) @RequestBody JsonMergePatch objectAsJsonMergePatch) {
        ...
        public ResponseEntity<Void> delete(
                @ApiParam(value = "Province's id value needed to delete Province", name = "id", type = "integer", required = true, example = "1") @PathVariable("id") Long id) {
