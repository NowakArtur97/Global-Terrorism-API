...
public class CityController extends GenericRestControllerImpl<CityModel, CityDTO, CityNode> {
        ...
        public ResponseEntity<CityModel> findById(
                @ApiParam(value = "City's id value needed to retrieve details", name = "id", type = "integer", required = true, example = "1") @PathVariable("id") Long id) {
        ...
        @Override
        @ApiOperation(value = "Update City' fields using Json Patch", notes = "Update City's fields using Json Patch", consumes = PatchMediaType.APPLICATION_JSON_PATCH_VALUE)
        @ApiResponses({
...
//     uniqueness)
        ...
        @Override
        @ApiOperation(value = "Update City's fields using Json Merge Patch", notes = "Update City's fields using Json Merge Patch", consumes = PatchMediaType.APPLICATION_JSON_MERGE_PATCH_VALUE)
        @ApiResponses({
                @ApiResponse(code = 200, message = "Successfully updated City's fields", response = CityModel.class),
                @ApiResponse(code = 400, message = "Incorrectly entered data", response = ErrorResponse.class)})
        public ResponseEntity<CityModel> updateFields(
                @ApiParam(value = "Id of the City being updated", name = "id2", type = "integer", required = true, example = "1") @PathVariable("id2") Long id,
                @ApiParam(value = "City's fields to update", name = "city", required = true) @RequestBody JsonMergePatch objectAsJsonMergePatch) {
        ...
        public ResponseEntity<Void> delete(
                @ApiParam(value = "City's id value needed to delete City", name = "id", type = "integer", required = true, example = "1") @PathVariable("id") Long id) {
